...
func (c *pg) CreateSchema(db, role, schema string, logger logr.Logger) error {
	...
	if err != nil {
		logger.Info("failed to connect to PostgreSQL server with database %s: %s", db, err.Error())
		return err
	}
	defer tmpDb.Close()
...
func (c *pg) DropDatabase(database string, logger logr.Logger) error {
	...
	//Disable connection
	_, err := c.db.Exec(fmt.Sprintf(DISABLE_CONNECTION, database))
	if err != nil {
		return err
	}
	// Close active connection
	_, err = c.db.Exec(fmt.Sprintf(CLOSE_CONNECTION, database))
	if err != nil {
		return err
	}
